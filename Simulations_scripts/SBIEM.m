...
% Implemented as a two-pass explicit scheme (SECOND ORDER):
...
%       %YIN:
%       oX.Slip(:,ito:end)= repmat(u(OX_IX),1,(NT-ito+1));
%       oX.SlipRate(:,ito:end)= repmat(v(OX_IX),1,(NT-ito+1));
%       oX.Stress(:,ito:end)  = repmat(stress1(OX_IX),1,(NT-ito+1)) ;
%       oX.Strength(:,ito:end)= repmat(strength(OX_IX),1,(NT-ito+1)) ;
...
%                         v(OX_IX)-VDYN,v_old(OX_IX)-VDYN, it,DT);
 ...
 % full time sampled outputs
  ...
  %% stop computing when rupture stop (YIN)
  if it>100 & max(v)==0
      Ixx=Ixx+1;
      if Ixx>50
          NTx=size(oX.Slip,2);
          oX.Slip(:,ito:end)= repmat(u(OX_IX),1,(NTx-ito+1));
          oX.SlipRate(:,ito:end)= repmat(v(OX_IX),1,(NTx-ito+1));
          oX.Stress(:,ito:end)  = repmat(stress1(OX_IX),1,(NTx-ito+1)) ;
          oX.Strength(:,ito:end)= repmat(strength(OX_IX),1,(NTx-ito+1)) ;
